...
use crate::{AsyncBlockDevice, Error, FileSystem, Scratch};

impl<D: AsyncBlockDevice> FileSystem<D> {
    /// Total allocatable bytes in the mounted volume.
    pub async fn capacity_bytes(&self) -> u64 {
    ...
    pub async fn free_space_bytes(
        &mut self,
        scratch: &mut Scratch<'_>,
    ) -> Result<u64, Error<D::Error>> {
        if self.free_clusters.is_none() {
            self.refresh_free_clusters(scratch).await?;
        }
    ...
    pub async fn refresh_free_space(
        &mut self,
        scratch: &mut Scratch<'_>,
    ) -> Result<u64, Error<D::Error>> {
        self.refresh_free_clusters(scratch).await?;
        self.free_space_bytes(scratch).await
    }

    pub(crate) async fn refresh_free_clusters(
        &mut self,
        scratch: &mut Scratch<'_>,
        ...
        while remaining_bits != 0 {
            let (lba, offset) = self.bitmap_location(byte, scratch).await?;
            self.device
    ...
    pub(crate) async fn set_fat(
        ...
        let lba = g.fat_lba_for_byte(byte).ok_or(Error::Corrupt)?;
    ...
    async fn bitmap_location(
        ...
        let cluster = self
            .cluster_at(bitmap.first_cluster, byte / cluster_bytes, scratch)
            .await?;
    ...
    pub(crate) async fn allocate_cluster(
        &mut self,
        scratch: &mut Scratch<'_>,
    ) -> Result<u32, Error<D::Error>> {
        self.allocate_cluster_preferred(2, scratch).await
    }
    ...
    pub(crate) async fn allocate_cluster_preferred(
        &mut self,
        preferred: u32,
        scratch: &mut Scratch<'_>,
    ) -> Result<u32, Error<D::Error>> {
        ...
        for cluster in first..end {
            if let Some(cluster) = self.claim_free_cluster(cluster, scratch).await? {
                return Ok(cluster);
            }
        }
    ...
    async fn claim_free_cluster(
        &mut self,
        cluster: u32,
        scratch: &mut Scratch<'_>,
    ) -> Result<Option<u32>, Error<D::Error>> {
        let g = self.volume.geometry();
        if !self.bitmap_used(cluster, scratch).await? {
            ...
            self.set_bitmap(cluster, true, scratch).await?;
            self.set_fat(cluster, 0xffff_ffff, scratch).await?;
